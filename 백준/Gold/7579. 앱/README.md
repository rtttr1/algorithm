...
### 풀이
총 메모리의 제한과 각각의 비용이 있기 때문에 배낭문제를 떠올려야 한다.
백트래킹으로 풀어보려다가 n이 100이기 때문에 2^100 의 상황은 1초를 넘기므로 패스했다.
이때 기본 배낭문제처럼 최대 메모리를 세로로 두면 10,000,000까지 계산을 해야하므로 메모리초과 or 시간 초과가 뜰것이다.
따라서 세로를 최대 코스트로 두고 접근을 해야지 풀 수 있는 문제였다.
항상 기본 알고리즘을 떠올리고, 이 알고리즘으로 온몸 비틀기를 생각해보는 습관을 들여야 되는것 같다.
